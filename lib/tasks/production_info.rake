...
namespace :bsc2 do
	...
	task :production_info_hourly => :environment do
		generate_consultancy_projects_data
	end

	task :production_info_daily => :environment do
		user_profiles
		time_entries
		checkpoints
		time_entries_daily
	...
	task :production_info_monthly => :environment do
		profiles_cost
		time_entries_years
		time_entries_last_year
		get_projects_with_cost_overrun_last_year
	...
	def user_profiles
		...
		users = User.find_by_sql(User.active.to_sql + " UNION " + User.joins(:profiles).to_sql)
		users.each do |user|
	...
	def generate_consultancy_projects_data
		...
		generate_csv("cons_projects_data", results)
	end
